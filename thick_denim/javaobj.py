...
class JavaObject(object):
    ...
    def __eq__(self, other):
        ...
        for name in self.classdesc.fields_names:
            res = res and getattr(self, name) == getattr(other, name)
        return res
...
class JavaObjectUnmarshaller(JavaObjectConstants):
    ...
    def readObject(self):
        try:
            opcode, res = self._read_and_exec_opcode(
                ident=0
            )  # TODO: add expects
            ...
            if len(the_rest):
                log_error(
                    "Warning!!!!: Stream still has %s bytes left. Enable debug mode of logging to see the hexdump."
                    % len(the_rest)
                )
    ...
    def _readStreamHeader(self):
        ...
        if magic != self.STREAM_MAGIC or version != self.STREAM_VERSION:
            raise IOError(
                "The stream is not java serialized object. Invalid stream header: %04X%04X"
                % (magic, version)
            )
    ...
    def _readStruct(self, unpack):
        ...
        if len(ba) != length:
            raise RuntimeError(
                "Stream has been ended unexpectedly while unmarshaling. (%d vs %d)"
                % (len(ba), length)
            )
    ...
    def do_classdesc(self, parent=None, ident=0):
        ...
        (serialVersionUID, newHandle, classDescFlags) = self._readStruct(
            ">LLB"
        )
        ...
        log_debug(
            "Serial: 0x%X newHandle: 0x%X. classDescFlags: 0x%X"
            % (serialVersionUID, newHandle, classDescFlags),
            ident,
        )
        ...
        for fieldId in range(length):
            ...
            if field_type == self.TYPE_ARRAY:
                opcode, field_type = self._read_and_exec_opcode(
        ...
        # superClassDesc
        opcode, superclassdesc = self._read_and_exec_opcode(
            ident=ident + 1,
    ...
    def do_object(self, parent=None, ident=0):
        ...
        log_debug(
            "java_object.annotations just after instantination: "
            + str(java_object.annotations),
            ident,
        )
        ...
        if (
            ...
            raise NotImplementedError(
                "externalContents isn't implemented yet"
            )  # TODO:
        ...
        if (
            ...
            # objectAnnotation
            log_debug(
                "java_object.annotations before: "
                + str(java_object.annotations),
                ident,
            )
            while opcode != self.TC_ENDBLOCKDATA:
                opcode, obj = self._read_and_exec_opcode(
                    ident=ident + 1
                )  # , expect=[self.TC_ENDBLOCKDATA, self.TC_BLOCKDATA, self.TC_OBJECT, self.TC_NULL, self.TC_REFERENCE])
                if opcode != self.TC_ENDBLOCKDATA:
                    java_object.annotations.append(obj)
                log_debug("objectAnnotation value: " + str(obj), ident)
            log_debug(
                "java_object.annotations after: "
                + str(java_object.annotations),
                ident,
            )
    ...
    def do_enum(self, parent=None, ident=0):
        ...
        opcode, enumConstantName = self._read_and_exec_opcode(
            ident=ident + 1, expect=[self.TC_STRING, self.TC_REFERENCE]
        )
    ...
    def _convert_char_to_type(self, type_char):
        ...
        if typecode in self.TYPECODES_LIST:
            ...
            raise RuntimeError(
                "Typecode %s (%s) isn't supported." % (type_char, typecode)
            )

    def _add_reference(self, obj, ident=0):
        log_debug(
            "## New reference handle 0x%X"
...
class JavaObjectMarshaller(JavaObjectConstants):
    ...
    def writeObject(self, obj):
        ...
        if isinstance(obj, JavaArray):
            ...
            raise RuntimeError(
                "Object serialization of type %s is not supported."
                % str(type(obj))
            )
    ...
    def write_object(self, obj, parent=None):
        ...
        if (
            ...
            for annot in obj.annotations:
                log_debug(
                    "Write annotation %s for %s" % (repr(annot), repr(obj))
                )
                if annot == None:
                    self.write_null()
                else:
    ...
    def write_classdesc(self, obj, parent=None):
        ...
        self._writeStruct(
            ">LLB", 1, (obj.serialVersionUID, obj.handle, obj.flags)
        )
        self._writeStruct(">H", 1, (len(obj.fields_names),))

        for name, type in zip(obj.fields_names, obj.fields_types):
            self._writeStruct(">B", 1, (self._convert_type_to_char(type),))
