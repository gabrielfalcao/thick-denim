...
def get_matching_issue_type(issue_type_name: str, issue_types: List[JiraIssueType]):
    types = issue_types.filter_by('name', issue_type_name)
    if not types:
        if not issue_types:
            raise RuntimeError(f'issue_types argument cannot be empty: {issue_type_name}')
        else:
            raise RuntimeError(f'no matching issue type {issue_type_name!r} in {issue_types}')
    target_task_type = types[0]
    if target_task_type.name != issue_type_name:
        raise RuntimeError(f'issue_type should be {issue_type_name}: {target_task_type}')
...
def transfer_tasks_without_epic(
        client, source_project, target_project):
    ...
    for source_task in source_issues_without_epic:
        ...
        print(f'transfering epic-less {source_task.key} "{source_task.summary}" to {target_project.key}')
        ...
        if transferred_task:
            print(f'\t\ttransfered task {transferred_task.key}: {transferred_task.summary}')
...
def transfer_epics_and_subtasks_to_another_project(
    ...
    for epic in epics:
        ...
        print(f'transfering Epic {epic.key}: {epic.summary} to project {target_project.key}: {target_project.name}')
        fields = {
        ...
        target_task_type = get_matching_issue_type(epic.issue_type_name, issue_types)
        ...
        if target_epic:
            ...
            for source_task in TASKS_BY_EPIC[epic]:
                ...
                print(f'\t\ttransfering {source_task.key} "{source_task.summary}" to {target_project.key}')
                target_task_type = get_matching_issue_type(source_task.issue_type_name, issue_types)
                try:
                    transferred_task = client.get_or_create_issue_by_summary(
                    ...
                    print(f'\033[1;31mfailed to transfer task{source_task.key} {source_task.summary!r}: {e}\033[0m')
                if transferred_task:
                    print(f'\t\ttransfered task {transferred_task.key}: {transferred_task.summary}')
